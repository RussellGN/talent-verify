...
class TestEndpointsSecurity(TestSetup):
   ...
   def test_can_register_with_proper_data(self):
      res = self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
      self.assertEqual(res.status_code, status.HTTP_201_CREATED)

   def test_cannot_register_without_providing_employer_admin_credentials(self):
      res = self.client.post(self.register_employer_url, {'employer': self.employer_data}, format='json')
   ...
   def test_cannot_register_without_providing_employer_admin_username(self):
      res = self.client.post(self.register_employer_url, {
         'employer-admin': self.credentials_no_username, 
         'employer': self.employer_data
      }, format='json')
      self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

   def test_cannot_register_without_providing_employer_admin_password(self):
      res = self.client.post(self.register_employer_url, {
         'employer-admin': self.credentials_no_password, 
         'employer': self.employer_data
      }, format='json')
      self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
   ...
   def test_can_get_employer_details_with_valid_auth_token(self):
      registration_res = self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
   ...
   def test_can_login_with_correct_credentials(self):
      self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
      res = self.client.post(self.login_employer_url, self.credentials, format='json')
      self.assertEqual(res.status_code, status.HTTP_200_OK)
   ...
   def test_cannot_login_without_providing_employer_admin_username(self):
      self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
      res = self.client.post(self.login_employer_url, self.credentials_no_username, format='json')
      self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

   def test_cannot_login_without_providing_employer_admin_password(self):
      self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
      res = self.client.post(self.login_employer_url, self.credentials_no_password, format='json')
      self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
   ...
   def test_can_update_employer_with_proper_data_and_valid_auth_token(self):
      registration_res = self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
      token = registration_res.data['token']
      res = self.client.patch(self.patch_employer_url, self.employer_and_employer_admin_patch_data, format='json', headers={'Authorization': f'Token {token}'})
      self.assertEqual(res.status_code, status.HTTP_200_OK)

   def test_cannot_update_employer_without_auth_token(self):
      res = self.client.patch(self.patch_employer_url, self.employer_and_employer_admin_patch_data, format='json')
      self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

   def test_cannot_update_employer_with_invalid_auth_token(self):
      res = self.client.patch(self.patch_employer_url, self.employer_and_employer_admin_patch_data, format='json', headers={'Authorization': f'Token invalid_token'})
      self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
   ...
   def test_can_post_employee_with_valid_auth_token_and_valid_data(self):
      ...
      res = self.client.post(self.handle_employees_url, self.employees_data, format='json', headers={'Authorization': f'Token {token}'})
      self.assertEqual(res.status_code, status.HTTP_201_CREATED)

   def test_cannot_post_employee_without_auth_token(self):
      res = self.client.post(self.handle_employees_url, self.employees_data, format='json')
      self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

   def test_cannot_post_employee_with_invalid_auth_token(self):
      res = self.client.post(self.handle_employees_url, self.employees_data, format='json', headers={'Authorization': f'Token invalid-token'})
      self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
   ...
   def test_can_patch_employee_with_valid_auth_token_and_valid_data(self):
      registration_res = self.client.post(self.register_employer_url, self.employer_and_employer_admin_registration_data , format='json')
      token = registration_res.data['token']
      self.client.post(self.handle_employees_url, self.employees_data, format='json', headers={'Authorization': f'Token {token}'})

      res = self.client.patch(self.handle_employees_url, self.employees_patch_data, format='json', headers={'Authorization': f'Token {token}'})
      self.assertEqual(res.status_code, status.HTTP_200_OK)
   ...
   def test_cannot_patch_employee_without_auth_token(self):
      ...
      res = self.client.patch(self.handle_employees_url, self.employees_patch_data, format='json')
      self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
   ...
   def test_cannot_patch_employee_with_invalid_auth_token(self):
      ...
      res = self.client.patch(self.handle_employees_url, self.employees_patch_data, format='json', headers={'Authorization': f'Token invalid-token'})
      self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
   ...
   def test_can_remove_employee_with_valid_auth_token_and_valid_id(self):
      ...
      creation_res = self.client.post(self.handle_employees_url, self.employees_data, format='json', headers={'Authorization': f'Token {token}'})
